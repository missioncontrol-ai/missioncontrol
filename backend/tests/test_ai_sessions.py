...
class AiSessionRouterTests(unittest.TestCase):
    ...
    def test_create_session_returns_valid_id(self):
        ...
        self.assertEqual(session["runtime_kind"], "claude_code")
    ...
    def test_create_and_turn_flow(self):
        ...
        # Turn submission: runtime will be unavailable in tests, expect graceful error event.
        ...
        # When runtime is unavailable the gateway emits runtime_error
    ...
    def test_invalid_runtime_rejected(self):
        req = _request("owner@example.com")
        with self.assertRaises(HTTPException) as ctx:
            _run(ai_router.create_session(ai_router.AiSessionCreate(title="bad", runtime_kind="invalid"), req))
