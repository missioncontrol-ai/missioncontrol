...
```sh
cd mc-mesh && cargo test
```

End-to-end acceptance (requires running backend + all three CLIs on PATH):
```sh
mc-mesh/scripts/e2e-test.sh
```

## Installation

```sh
mc-mesh/scripts/install.sh       # builds from source, installs to ~/.cargo/bin/
mc mesh up                       # starts daemon, optionally installs systemd unit
```

Or via the curl bootstrap in `integrations/mc/`:
```sh
